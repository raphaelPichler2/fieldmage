...
class GiesKanne extends Skill{
  GiesKanne(float posX, float posY){
    super(posX,posY);
    img=gieskanneI;
    manaUse=4.0;
    cooldown=120*2;
  ...
  void baseicActivate(){
    ...
    for(int i=0;i<smallMap.size();i++){
      if(smallMap.get(i) instanceof Sapling && distance(player.posX,player.posY,smallMap.get(i).posX,smallMap.get(i).posY) < (player.size+10)){
        MapObj t = new Tree(smallMap.get(i).posX,smallMap.get(i).posY-80);
        if(random(100)<5) t= new MagicTree(smallMap.get(i).posX,smallMap.get(i).posY-80);
        smallMap.add(t);
        bigMap.add(t);
    ...
    if(newPlant){
      MapObj t = new Sapling(player.posX,player.posY);
      if(random(100)<30) t = new Mushroom(player.posX,player.posY);
      smallMap.add(t);
      bigMap.add(t);
    }
  }
