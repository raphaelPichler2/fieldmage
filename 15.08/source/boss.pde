class Boss extends Prey{
  ...
  Boss(color c,float size,float maxHp,float xpSaved,float walkSpeed,float runSpeed, float dmg, float droprate){
    ...
    this.maxHp=this.maxHp*1.1*(float)Math.pow(1.12,bossKills);//0.9*this.maxHp+0.1*this.maxHp*enemyStrength/0.3;//this.maxHp*0.5+his.maxHp*0.5/enemyStrength*0.3*Math.pow(1.35,bossesKilled)
    hp=this.maxHp;
    ccRes=20;
    //spawnRate*=0.9;
  }
...
class Fish extends Boss{
  ...
  Fish(){
    super(#6F679D,140,800,0,1,1.2,50,0);//color c,float size,float maxHp,float xpSaved,float walkSpeed,float runSpeed, float dmg, float droprate
    body = new Body(this);
...
class BeeBoss extends Boss{ 
  ...
  BeeBoss(){
    super(#6F679D,140,700,0,1,0.9,30,0);//color c,float size,float maxHp,float xpSaved,float walkSpeed,float runSpeed, float dmg, float droprate
    shotSpeed=60;
    maxPhase=4;
    timeInPhase=120*4;
    phase=0;
