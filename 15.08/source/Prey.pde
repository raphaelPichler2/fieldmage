...
void spawnEnemy(boolean casual){
  ...
  spawnRate=0.7+1.25*(float)Math.log(1+0.23*time/120.0/60.0);
  dropChance=0.7/spawnRate ;
  if(prey.size()>1200 || activeBoss!=null){
    if(0.4+0.02*time/120.0/60.0<1) spawnRate*=0.4+0.025*time/120.0/60.0;
  }
  enemyStrength=0.32*(float)Math.pow(1.1,time/120.0/60.0) + 0.059*time/120.0/60.0;
  enemyDmg=0.5+  0.5*0.35*(float)Math.pow(1.1,time/120.0/60.0) + 0.059*time/120.0/60.0;
  ...
  for(int i=0;i<=4;i++){
    ...
    if(random(120)<8.0/10 || casual){
      float diffSpawn=1+1.4*time/120.0/60.0;
...
class Prey{
  ...
  void die(){
    ...
    if(random(100)<droprate*0.3) pickups.add(new Coin(posX+10,posY-50));
...
class BigShooter extends Prey{
  ...
  BigShooter(){
    super(255,75,70,0,0.5,0.8,40,35);//color c,float size,float maxHp,float xpSaved,float walkSpeed,float runSpeed, float dmg, float droprate
    shotSpeed=46;
    weight=1.4;
  }
...
class Wanderer extends Prey{
  ...
  void shoot(){
    if(hp<maxHp && activeBoss==null) shoot2(12,15,2.5);
  }
...
class Healer extends Prey{
  ...
  Healer(){
    super(255,75,120,0,0.5,0.8,20,60);//color c,float size,float maxHp,float xpSaved,float walkSpeed,float runSpeed, float dmg, float droprate
    weight=1.8;
  }
  void body(){
    pushMatrix();
...
class SpeedBuffer extends Prey{
  ...
  SpeedBuffer(){
    super(255,65,120,0,0.5,1.3,30,35);//color c,float size,float maxHp,float xpSaved,float walkSpeed,float runSpeed, float dmg, float droprate
    weight=1.0;
  }
  void body(){
    pushMatrix();
    ...
    for(int i=0;i<prey.size();i++){
      if(distance(posX,posY,prey.get(i).posX,prey.get(i).posY) < range/2 ){
        if(!(prey.get(i) instanceof SpeedBuffer) ){
          prey.get(i).posX+=prey.get(i).speedX*1.0;
          prey.get(i).posY+=prey.get(i).speedY*1.0;
        }
      }
    }
  }
...
class Sprayer extends Prey{
  Sprayer(){
    super(255,110,150,0,0.4,0.6,9,70);
    shotSpeed=200;
    range=650;
  }
  
  void shoot(){
    shootSpray(12,4.5,60);
  }
  void run(){
    float dist=distance(posX,posY,player.posX,player.posY);
    speedX = runSpeed*(player.posX-posX)/dist;
