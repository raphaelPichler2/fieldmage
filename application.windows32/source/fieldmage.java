...
public void createMap(){
  ...
  for(int i = 0; i<16100;i++){//+1000 pro item 23*700
...
public void spawnEnemy(boolean casual){
  ...
  spawnRate=0.7f+6.0f*(float)Math.log(1+ (1/(6.0f*5)) *time/120.0f/60.0f);
  dropChance=0.7f/spawnRate ;
  if(prey.size()>1200 || activeBoss!=null){
    if(0.4f+0.03f*bossKills<1) spawnRate*=0.4f+0.03f*bossKills;
  }
  enemyStrength=0.33f*(float)Math.pow(1.12f,time/120.0f/60.0f) + 0.056f*time/120.0f/60.0f;
  enemyDmg=0.35f*0.33f+  0.65f*( 0.33f*(float)Math.pow(1.12f,time/120.0f/60.0f) + 0.056f*time/120.0f/60.0f );
...
class Prey{
  ...
  public void die(){
    ...
    if(random(100)<droprate*0.45f) pickups.add(new Coin(posX+10,posY-50));
...
class KillEnemiesFrameMission extends Mission{
  ...
  public void checkFinished(){
    ...
    if(kills.size()>score) score=kills.size();
  }
}
class Kill15EnemiesFrameMission extends KillEnemiesFrameMission{
  Kill15EnemiesFrameMission(){
    goal=15;
  }
}
...
class StatMission extends Mission{
  ...
  public void checkFinished(){
    if(player!=null){
      if(getStat()>=goal){ finished=true; money+=100;}
      if(getStat()>score) score=getStat();
    }
  }
  public float getStat(){return 10;}
}
...
class StatManaMission extends StatMission{
  ...
  public float getStat(){return 120*player.manaRegen;}
}
class StatRegenMission extends StatMission{
  ...
  public float getStat(){return 120*player.regen;}
}
...
class SkinDeath extends Skin{
  SkinDeath(){
    img=deathI;
    m=new Kill15EnemiesFrameMission();
  }
}
...
class Boss extends Prey{
  ...
  Boss(int c,float size,float maxHp,float xpSaved,float walkSpeed,float runSpeed, float dmg, float droprate){
    ...
    posX=player.posX+dist*cos(angle);
    posY=player.posY+dist*sin(angle);
    this.maxHp=this.maxHp*1.1f*(float)Math.pow(1.1f,bossKills);//0.9*this.maxHp+0.1*this.maxHp*enemyStrength/0.3;//this.maxHp*0.5+his.maxHp*0.5/enemyStrength*0.3*Math.pow(1.35,bossesKilled)
    hp=this.maxHp;
    ccRes=20;
    //spawnRate*=0.9;
  }
