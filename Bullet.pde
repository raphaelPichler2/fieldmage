...
class SchrottBullet extends Bullet{
  ...
  SchrottBullet(float dmg){
    super(0);
    this.dmg=dmg;
    maxRange=320+player.bulletSize*5;
...
class ElectricBolt extends BoltBullet{
  ElectricBolt(float dmg, float posX,float posY){
    super( dmg,  posX, posY, 600);
    this.size=52;
  }
    ...
    for(int i=0;i<prey.size();i++){    
      ...
      distance(posX,posY,prey.get(i).posX,prey.get(i).posY) < (size+prey.get(i).size)/2 ){
        ...
        if(p.hp>0) trigger();
          ...
          newEnemy();
          //new ElectricBolt(player.dmg, posX+random(-80,80),posY+random(-80,80));
        }
      }
    }
  }
  ...
  void skin(){
    turnImgD(blueBoltI,posX,posY,size*2,size*2,angle(0,0,speedX,speedY));
  }
}
...
class Gas extends Bullet{
  ...
  void draw(){
    timer--;
    if(timer<=0) bullets.remove(this);
    size+=(100.0+player.bulletSize)/120/2.7;
  ...
  void draw(){
    ...
    if(landed){
      ...
      for(int i=0;i<smallMap.size();i++){
        if((smallMap.get(i) instanceof Tree || smallMap.get(i) instanceof MagicTree) && distance(posX,posY,smallMap.get(i).posX,smallMap.get(i).posY) < size){
          TreeStump t = new TreeStump(smallMap.get(i).posX,smallMap.get(i).posY);
          smallMap.add(t);
          bigMap.add(t);
          
          if(random(100)<5 || smallMap.get(i) instanceof MagicTree) items.add(randomLoot(smallMap.get(i).posX,smallMap.get(i).posY));
          
          bigMap.remove(smallMap.get(i));
          smallMap.remove(i);
          i--;
        }
      }
