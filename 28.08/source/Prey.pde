...
void spawnEnemy(boolean casual){
  ...
  spawnRate=0.7+6.0*(float)Math.log(1+ (1/(6.0*5)) *time/120.0/60.0);
  dropChance=0.7/spawnRate ;
  if(prey.size()>1200 || activeBoss!=null){
    if(0.4+0.03*bossKills<1) spawnRate*=0.4+0.03*bossKills;
  }
  enemyStrength=0.33*(float)Math.pow(1.12,time/120.0/60.0) + 0.056*time/120.0/60.0;
  enemyDmg=0.35*0.33+  0.65*( 0.33*(float)Math.pow(1.12,time/120.0/60.0) + 0.056*time/120.0/60.0 );
...
class Sprayer extends Prey{
  Sprayer(){
    super(255,110,150,0,0.4,0.6,8,70);
    shotSpeed=200;
    range=650;
  }
